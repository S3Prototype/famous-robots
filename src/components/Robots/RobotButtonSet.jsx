...
const VoteButtonSet = (props, data) => {
  const handleVote = async () => {
    try {
      ...
      return data.setErrorMessage(
        `Error sending vote to server: ${err.message}`
      );
    }
  };
  ...
  return (
    <Button
      disabled={
        props.alreadyVotedList &&
        props.alreadyVotedList.includes(props.robot._id)
      }
      disableElevation
...
const AdminButtonSet = (props, data) => {
  const deleteRobot = async () => {
    try {
      ...
      return data.setErrorMessage(
        `Error trying to delete robot ${props.robot.name}: ${err.message}`
      );
    }
  };
  ...
  return (
    <Grid style={{ width: "100%", columnGap: 20 }} container justify="center">
      <Button
        ...
        color="primary"
        onClick={() =>
          props.convertToEditCard({ type: "add", id: props.robot._id })
        }
      >
        EDIT
      </Button>
